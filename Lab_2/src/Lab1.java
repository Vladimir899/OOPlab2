...
public class Lab1 {
    public static void main(String[] args) {
        // работа с точками в трехмерном пространстве1
        ...
        // идём по циклу и считываем координаты точек
        for(int i = 0; i<len; i++){
            points[i] = new Point3d();
            // x
            System.out.printf("Введите координату X %d-й точки: ", i+1);
        ...
        boolean eql = false;
        // проверяем три точки на равенство
        if (points[0].compare(points[1])) {
            eql = true;
            System.out.printf("Ошибка! Точка 1 с координатами (%f;%f;%f) и точка 2 с координатами (%f;%f;%f) равны",
                    points[0].getX(), points[0].getY(), points[0].getZ(), points[1].getX(), points[1].getY(), points[1].getZ());
        }
        if (points[1].compare(points[2])) {
            eql = true;
            System.out.printf("Ошибка! Точка 2 с координатами (%f;%f;%f) и точка 3 с координатами (%f;%f;%f) равны",
                    points[1].getX(), points[1].getY(), points[1].getZ(), points[2].getX(), points[2].getY(), points[2].getZ());
        }
        if (points[2].compare(points[0])) {
            eql = true;
            System.out.printf("Ошибка! Точка 3 с координатами (%f;%f;%f) и точка 1 с координатами (%f;%f;%f) равны",
                    points[2].getX(), points[2].getY(), points[2].getZ(), points[0].getX(), points[0].getY(), points[0].getZ());
        }
        // если у нас нет ни одни точки, не равной друг другу - находим площаль треугольника
        if (eql == false){
